...
class VariationalDeepEmbedding:

    def __init__(self, input_dim, latent_dim, conv_layers, full_layers, px_z, lr, batch_size, save_dir=None):
        ...
        # declare placeholders
        self.x_ph = tf.placeholder(tf.float32, [None] + self.input_dim)
        self.z_ph = tf.placeholder(tf.float32, [None, self.latent_dim, self.k])
        self.training = tf.placeholder(tf.bool)
        self.dropout_ph = tf.placeholder(tf.float32)
        ...
        # build encoder network
        self.z_mu, self.z_std, self.z_pi = self.encoder()

        # build decoder network
        self.x_hat_mu, self.x_hat_var = self.decoder()
    ...
    def postive_real_activation(self, z):

        # convert to Re+ with continuous differentiability
    ...
    def encoder(self):
        ...
        # flatten features to vector
        x = tf.contrib.layers.flatten(x)
        self.encoder_flat_shapes = x.get_shape().as_list()[1:]
        ...
        with tf.variable_scope('EncOut') as scope:
            ...
            # reshape to [K, dim(z)]
            z_mu = tf.reshape(z_mu, [-1, self.latent_dim, self.k])

            # compute z std
            z_std = tf.layers.dense(inputs=x,
                                    units=(self.latent_dim * self.k),
                                    activation=self.postive_real_activation,
                                    use_bias=True,
                                    kernel_initializer=self.weight_init,
                                    bias_initializer=tf.constant_initializer(0.0),
                                    name='zSTD')
            ...
            # add minimum value
            z_std += self.std_min_value

            # reshape to [K, dim(z)]
            z_std = tf.reshape(z_std, [-1, self.latent_dim, self.k])

            # compute pi
            z_pi = tf.layers.dense(inputs=x,
                                   units=self.k,
            ...
            # reshape to [K, 1]
            z_pi = tf.expand_dims(z_pi, axis=-1)

            # print size information
            assert z_mu.get_shape().as_list()[1:] == z_std.get_shape().as_list()[1:]
            print('EncOut: ' + str(input_dim) + ' --> ' + str(z_mu.get_shape().as_list()[1:]))

        # return latent variables
        return z_mu, z_std, z_pi
    ...
    def decoder(self):
        ...
        # take expectation w.r.t. clusters
        x = tf.squeeze(tf.matmul(x, self.z_pi), axis=-1)
        ...
        with tf.variable_scope('DecOut') as scope:
            ...
            # print size information
            assert self.input_dim == x_hat_mu.get_shape().as_list()[1:]
            print('DecConv{:d}: '.format(len(decoder_conv_layers)) + str(input_dim) + ' --> ' + str(self.input_dim))
    ...
    def loss_operation(self):
        ...
        with tf.variable_scope('Loss') as scope:
            ...
            # compute mse
            sq_diff = tf.squared_difference(self.x_ph, self.x_hat_mu)
            ...
            if self.px_z == 'Gaussian':
                ...
                # compute reconstruction loss: -E[p(x|z)]
                log_det = -0.5 * tf.reduce_sum(tf.log(2 * np.pi * self.x_hat_var), axis=1)
                log_exp = -0.5 * tf.reduce_sum(tf.layers.flatten(sq_diff) / self.x_hat_var, axis=1)
                rec_loss = -tf.reduce_mean(log_exp + log_det) * tf.constant(self.n_inputs, dtype=tf.float32)

            # Bernoulli output treatment
            elif self.px_z == 'Bernoulli':

                # compute logistic loss
                rec_loss = -tf.reduce_mean(self.x_ph * tf.log(self.x_hat_mu + 1e-6) +
                                           (1 - self.x_ph) * tf.log(1 - self.x_hat_mu + 1e-6))
                rec_loss = rec_loss * tf.constant(self.n_inputs, dtype=tf.float32)
    ...
    def feed_dict_samples(self, x_batch, training):
        ...
        # take center and width samples from standard normal: N(0,I)
        z_samps = np.random.normal(size=[batch_size, self.latent_dim, self.k])
    ...
    def train(self, x_train, x_test):
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.num_epochs):
                ...
                # get training batches
                batches = get_batches(x_train.shape[0], self.batch_size)

                # configure writer to write five times for this epoch
                write_interval = np.round(len(batches) / 5)

                # loop over the batches
                for j in range(len(batches)):

                    # load a feed dictionary
                    feed_dict = self.feed_dict_samples(x_train[batches[j]], True)

                    # writing to tensor board?
                    if np.mod(j, write_interval) == 0 and train_writer is not None:

                        # run training, loss, accuracy, and summary
                        _, loss, mse, summary = sess.run([self.train_op,
                                                          self.loss_op,
                                                          self.mse_op,
                    ...
                    # not writing to tensor board
                        ...
                        # run just training and loss
                        _, loss, mse = sess.run([self.train_op, self.loss_op, self.mse_op], feed_dict=feed_dict)
                    ...
                    # print update
                    per = 100 * (j + 1) / len(batches)
                    update_str = 'Epoch %d, Percent Complete = %f%%, MSE = %f' % (i, per, mse)
                    print('\r' + update_str, end='')

                # save total loss
                total_loss_hist.append(loss)

                # compute and save reconstruction and latent loss
                rec_loss, lat_loss = sess.run([self.rec_loss_op, self.lat_loss_op], feed_dict=feed_dict)
                rec_loss_hist.append(rec_loss)
                lat_loss_hist.append(lat_loss)

                # print time for epoch
                stop = time.time()
                print('\nTime for Epoch = {:f}, Rec. Loss = {:.2f}, Latent Loss = {:.2f}'.format(stop - start,
                                                                                                 rec_loss,
                                                                                                 lat_loss))
                ...
                # generate test image
                feed_dict = self.feed_dict_samples(x, False)

                # run the optimizer
                x_hat = sess.run([self.x_hat_mu], feed_dict=feed_dict)[0]
                ...
                # plot the loss
                fig_loss.clf()
                epochs = np.arange(1, len(total_loss_hist) + 1)
                sp = fig_loss.add_subplot(1, 1, 1)
                plt.plot(epochs, total_loss_hist, label='total')
                plt.plot(epochs, rec_loss_hist, label='reconstruction')
                plt.plot(epochs, lat_loss_hist, label='latent')
                plt.legend()
                sp.set_title('Objective: Epoch = %d' % np.argmin(total_loss_hist))
                sp.set_xlabel('Epoch')
                sp.set_ylabel('Batch Loss at Start of Epoch')
            ...
            # get test loss
            test_loss = self.test_loss(sess, x_test)
    ...
    def test_loss(self, sess, x_test):

        # evaluate test error
        test_error = []
        for i in range(x_test.shape[0]):
            feed_dict = self.feed_dict_samples(np.expand_dims(x_test[i], axis=0), False)
            loss = sess.run([self.loss_op], feed_dict=feed_dict)
            test_error.append(loss[0])
